...
## Flow Trigger: Automatic Plan Mode Entry

When the user provides an initial project idea or requirement, **automatically enter plan mode** to begin the structured planning process.

**Trigger Detection:**
- User describes a new software system to be built
- User asks to "build" or "create" something requiring multiple steps
- User provides requirements that need analysis and breakdown

**Automatic Entry Action:**
Use `EnterPlanMode` immediately after receiving the initial project concept. Do not ask for confirmation - plan mode IS the planning process.

---

## Post-Plan Mode: Autonomous Documentation Generation

When plan mode exits (after `ExitPlanMode` is called with user approval), **autonomously generate ONLY the planning documentation**:

1. Create `.claude/CLAUDE.md` with project context
2. Create `.claude/docs/` directory if it doesn't exist
3. Generate initial `PROJECT-PLAN.md` with gathered information

**⚠️ CRITICAL: STOP AFTER DOCUMENTATION**
- DO NOT create source files
- DO NOT install dependencies
- DO NOT start building
- WAIT for user to run setup-env.sh manually
- WAIT for user to start Ralph Loop manually
...
```
┌─────────────────────────────────────────────────────────────────┐
│ AUTOMATIC: Enter Plan Mode on Initial User Input               │
├─────────────────────────────────────────────────────────────────┤
...
│   - User approval before Ralph Loop                             │
├─────────────────────────────────────────────────────────────────┤
│ Phase 0: Environment Setup (Pre-Ralph)                          │
...
**Note:** This phase occurs during plan mode. Use `AskUserQuestion` to gather requirements interactively.
...
## Phase 1 Complete: Generate Project CLAUDE.md (AUTOMATIC)

**When plan mode exits** (after user approves via `ExitPlanMode`), **automously generate** the project-specific CLAUDE.md file. This happens without additional user prompting.
...
### 1.2 Automatic Generation on Plan Mode Exit

When plan mode completes (user approves via `ExitPlanMode`), immediately generate:

```markdown
# Automatic Post-Plan Actions
...
[Using the template in section 1.1 above, populate with gathered information]

## 3. Generate .claude/docs/PROJECT-PLAN.md
[Using the template in Phase 2, populate with gathered information]

## 4. Confirm to User
"Project documentation structure created. Ready to proceed with Phase 3: Specification Breakdown."
```

### 1.3 Generate from Planning Documents
...
**Important:** This is created AUTOMATICALLY when plan mode exits because it references the planning documents that are created throughout the planning process. The file should be updated as each phase completes to include references to new documentation.

---

## Phase 2: Project Plan Document (v1) (IN PLAN MODE)

**Note:** This phase also occurs during plan mode. Use `AskUserQuestion` for any clarifications needed.
